    ...
    static init() {
        ...
        if (!AuthService.isAuthenticated()) {
            ...
            if (!currentPath.endsWith('login.html') && 
                ...
                sessionStorage.setItem('shule_redirect_to', window.location.href);
                ...
                if (frontendIndex !== -1) {
                    ...
                    window.location.href = '/shule-cafe/frontend/auth/login.html';
                }
            }
        } else {
            ...
            if (currentPath.includes('/headmaster/') && !['headmaster', 'tenant_admin', 'school_admin', 'super_admin'].includes(userRole)) {
                console.warn('[AuthGuard] Unauthorized role path access attempt:', currentPath, 'Role:', userRole);
                AuthService.redirectBasedOnRole(userRole);
            } else if (currentPath.includes('/teacher/') && !['teacher', 'headmaster', 'tenant_admin', 'school_admin', 'super_admin'].includes(userRole)) {
                console.warn('[AuthGuard] Unauthorized role path access attempt:', currentPath, 'Role:', userRole);
                AuthService.redirectBasedOnRole(userRole);
            } else if (currentPath.includes('/super-admin/') && userRole !== 'super_admin') {
                console.warn('[AuthGuard] Unauthorized role path access attempt:', currentPath, 'Role:', userRole);
                AuthService.redirectBasedOnRole(userRole);
            } else if (currentPath.includes('/regional/') && !['regional_officer', 'super_admin'].includes(userRole)) {
                console.warn('[AuthGuard] Unauthorized role path access attempt:', currentPath, 'Role:', userRole);
                AuthService.redirectBasedOnRole(userRole);
            } else if (currentPath.includes('/parent/') && !['parent', 'guardian', 'super_admin', 'tenant_admin', 'school_admin', 'headmaster'].includes(userRole)) {
                console.warn('[AuthGuard] Unauthorized role path access attempt:', currentPath, 'Role:', userRole);
                AuthService.redirectBasedOnRole(userRole);
            }
        }
