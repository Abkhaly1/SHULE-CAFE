...
try {
    ...
    $stmt2 = $conn->prepare("
        INSERT INTO users 
        (id, school_id, user_code, full_name, email, phone, password_hash, temp_password, is_password_changed, first_login_completed, role, status) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 'tenant_admin', 'active')
    ");
    $stmt2->execute([
        $user_id,
        $school_id,
